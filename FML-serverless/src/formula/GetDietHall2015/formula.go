...
type ODEFittedModelValues struct {
	G      float64
	Psig   float64
	P      float64
	L_diet float64
...
type Result struct {
	...
	Fa                       []float64
	SM                       []float64
	UserDependantParameters  InitializationHall2015ODES.Result
	LastValuesODEFittedModel ODEFittedModelValues
}

func Calculate(age int64, gender Gender.Enum, heightCMS, pal, prop_pi, rmr, initialBWKg, initialFMKg, finalBW,
	carbsProportionNewDiet, fatProportionNewDiet, proteinProportionNewDiet float64, daysBetweenInitialAndFinal, predictionDays int64, step float64) (Result, error) {
	...
	result := Rk4onePhaseHypHall2015.Calculate(gender, heightCMS, initialFMKg, initialBWKg, pal, rmr, carbsProportionNewDiet, fatProportionNewDiet, proteinProportionNewDiet, prop_pi,
	...
	for math.Abs(errorFactor) > 50 {
		...
		averageEI = float64(commons.GetRandomInt(averageEI_LOW, averageEI_UP))
		...
		result = Rk4onePhaseHypHall2015.Calculate(gender, heightCMS, initialFMKg, initialBWKg, pal, rmr, carbsProportionNewDiet, fatProportionNewDiet,
		...
		if (averageEI_UP - averageEI_LOW) < 1 {
			errorFactor = 0
			return Result{}, fmt.Errorf("There was an issue processing your weight data. Showing results based on your most recent verified entry")
		}
	...
	dietResult := Result{
		...
		UserDependantParameters: result.UserDependantParameters,
		LastValuesODEFittedModel: ODEFittedModelValues{
	...
	if predictionDays == 0 {
		...
		predictionResult := Rk4onePhaseHypHall2015.Calculate(gender, heightCMS, initialFMKg, initialBWKg, pal, rmr, carbsProportionNewDiet, fatProportionNewDiet,
