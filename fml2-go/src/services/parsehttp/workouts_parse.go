...
func GetWorkouts() ([]workouts.Workout, error) {
	client := &http.Client{}
	all := make([]workouts.Workout, 0)

	req, err := http.NewRequest("GET", "https://dev.api.fitmylife.net/parse/classes/Workout?limit=999", nil)
	if err != nil {
		return all, err
	}
...
func GetWorkoutSchedules() ([]workouts.WorkoutSchedule, error) {

	client := &http.Client{}
	schedules := make([]workouts.WorkoutSchedule, 0)
	...
	for _, value := range result.Results {
		...
		schedule := workouts.WorkoutSchedule{
			PrimaryGoal:     parseGoalWithParseName(raw["primaryGoal"].(string)),
			SecondaryGoal:   parseGoalWithParseName(raw["secondaryGoal"].(string)),
